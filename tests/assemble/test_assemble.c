...
void test_assembly(void)
{
	...
	TEST_ASSERT_EQUAL_INT16(ADDR_MODE_INDIRECT_Y_INDEX, addr_mask);

	instr->addr_bitflag = addr_mask & instr->addr_bitfield;
	TEST_ASSERT_EQUAL_INT16(ADDR_MODE_INDIRECT_Y_INDEX, instr->addr_bitflag);

	TEST_ASSERT_EQUAL_INT16(0xF1, assemble_SBC(instr->addr_bitflag));



	TEST_ASSERT_EQUAL_INT(ERROR_ILLEGAL_ADDRESSING_MODE, assemble_JSR(ADDR_MODE_ABSOLUTE_INDIRECT));
	...
	TEST_ASSERT_EQUAL_INT16(0x45, assemble_EOR(ADDR_MODE_ZERO_PAGE));
	TEST_ASSERT_EQUAL_INT16(0x45, assemble_EOR(ADDR_MODE_ZERO_PAGE));
	TEST_ASSERT_EQUAL_INT16(0x9D, assemble_STA(ADDR_MODE_ABSOLUTE_X));
